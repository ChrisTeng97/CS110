...
main:
    ...
    li a1, 5
    jal inc_arr
    jal check_arr # Verifies inc_arr and jumps to "failure" on failure  # passed
...
naive_pow_end:
    ...
    lw s0, 8(sp)
    lw a1, 0(sp)
    # lw a0, 4(sp) # dont write this (overwrite a0)!
    addi sp, sp, 12
    # END EPILOGUE
    ret
...
helper_fn:
    ...
    # add 3 lines (to prevent s0,t1 being used before?)
    addi sp, sp, -8
    sw t1, 0(sp)
    sw s0, 4(sp)
