...
dotp_manual_optimized (double *x, double *y, int arr_size)
{
  ...
  #pragma omp parallel
  {
    ...
    /*
    // but this works as slow as single-thread
    for (int i = thread_num*arr_size/num_threads; i <__min(arr_size, (thread_num+1)*arr_size/num_threads); i++){
      local_sum += x[i]*y[i];
    }
    */
    #pragma omp for
...
int main ()
{
  ...
  int num_threads = omp_get_max_threads (); // 16 on my windows machine
